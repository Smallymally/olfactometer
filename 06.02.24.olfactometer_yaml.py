...
def set_parameters(yml):
    if list(yml.keys())[0]!="parameters": # In python 3, dict_keys object is not subscriptable
       ...
       try:
         constant_flow_enabled=yml["parameters"]["constant_flow_enabled"]
       except:
         #cannot read constant flow enable/disable flag from yaml. default it to enabled
         constant_flow_enabled=True
       ...
       return port, constant_flow_rate, constant_flow_id, constant_flow_enabled, calibration, run, max_flow, pause
...
def run_expt(yml,ser,constant_flow_id,constant_flow_enabled):

    # 1) Open constant flow (if enabled)
    if constant_flow_enabled == True:
    ...
    # 4) Close the constant flow (always)
    cmd_str="setchannel "+str(constant_flow_id)
    ser_exec(ser,cmd_str)
...
def ser_exec(ser,cmd_str):
    ...
    try:
       ...
       print(f"ERROR: error writing to serial {e}")
    ...
    with open(args.input, 'r') as file:
        try:
          ...
          sys.exit()
           ...
           port, constant_flow_rate, constant_flow_id, constant_flow_enabled, calibration, run, max_flow,pause=set_parameters(yml)
           try:
           ...
           if calibration is True:
              ...
              if outcome is False:
                 ...
                 sys.exit()
           if run is True:
              run_expt(yml,ser,constant_flow_id,constant_flow_enabled)
